package com.gen_5_new; import static com.gen_5_new.Base.*;
...
public class Identify {
    ...
    private static String buffer, NameSubstanсe = "NH4(SO4OH)5";//(NH4)3(OH)8SO4 (NH4)3(SO4)8(OH5)7 (NH4)3SO4(OH5)7
    ...
    private static void Update(int code) {
        if(code == 1) {
            ...
            control = false; auxiliary1 = true; arrayChar = NameSubstanсe.toCharArray(); length = arrayChar.length;
        }
        if(code == 2) {
    ...
    public static int keyI = 0;
    public static void Identify() {
        Update(1);
        for(; all < 7; all++) {
            Update(2);
            if(all == 6 && arrayChar[length - 2] == ')') {
                length += 1;
                keyB = true;//Про скобки Надо удалить а точнее переделать
            }
            for(; length > almost && BracketBoolean; almost++, auxiliary1 = true) {
                System.out.println("========Новый цикл======== almost:" + almost);//TODO Delete
                if(keyB) {
                    if(id[6] != 1) { length++; }
                    if(keyI == 3) { buffer = ""; BracketBoolean = false;
                        if(id[6] != 1) { almost -= 1; }
                        else almost -= 2;
                    }
                    keyI++;
